...
| `TORPLEX_SEARCH_PROVIDER_FAILURE_LIMIT` | `8` | Consecutive unusable manifests allowed per provider and target before its remaining results are skipped, clamped to 1-10. This keeps a provider in play when its highest-ranked listings are stale. |
| `TORPLEX_SEARCH_METADATA_CANDIDATE_LIMIT` | `16` | Initial provider-diverse manifest candidates per target, clamped to 4-40. |
...
| `TORPLEX_SEARCH_VERIFIED_CANDIDATE_TARGET` | `2` | Number of scope-compatible manifests to collect per target before model selection, clamped to 1-8. Two normally provides a primary plus fallback without delaying the whole proposal. |
| `TORPLEX_SEARCH_METADATA_BUDGET_SECONDS` | `60` | Per-target adaptive manifest-discovery budget before Torplex reports no usable source, clamped to 30-600 seconds. |
...
2. Nova searches broad canonical title, year, and season terms through the configured provider allowlist. Quality tags are deliberately not included in provider queries because many healthy releases omit codec or resolution from searchable titles; Torplex applies the selected quality profile locally after retrieval. It first searches each series as a complete pack, verifies every filename in retrieved manifests, and expands a series into independent season targets when no verified pack covers the requested scope. Movies remain single targets.
...
4. The model sees only candidates whose file manifests were actually retrieved, whose deterministic season coverage matches the target, and whose release satisfies the selected quality profile. **Direct play** is the default profile: it requires positively identified H.264/AVC 8-bit SDR video from 480p through 1080p and rejects HEVC/H.265, HDR, 10-bit, over-1080p, and unknown-codec releases. This is the safest broadly compatible target for low-power Plex servers, although final Direct Play support still depends on the client, audio track, subtitles, and container. Balanced, compact, maximum-quality, and fully custom profiles remain available. The model may select one opaque candidate ID and up to three independently suitable verified fallbacks for each movie, complete series, or season, but it cannot invent a source URL. Accepted magnets reuse the cached metadata during intake.
5. When Plex credentials are configured, Find with AI reads actual Plex movie and episode media metadata. A movie or TV season that exists only in an incompatible codec is treated as needing a replacement; compatible seasons remain excluded from the search.
6. Each accepted result runs Smart Setup with a client-side concurrency limit of two. Season-scoped results receive explicit Plex season-folder instructions. If a previously unverified fallback is later needed, Torplex advances through the remaining reviewed sources. Every file selection, Plex path, organizer route, and post-download check remains manually editable.
7. A transactional bulk request validates the complete batch before writing torrent descriptors or the queue manifest.
...
Search progress and proposals are persisted in `BATCH_DIR/search-sessions.json`. Each browser tab keeps only its own opaque search ID in session storage, so reloading reconnects to that tab's search without exposing the newest proposal from another user or tab. Multiple authenticated clients can run independent searches concurrently. **Clear search** cancels that tab's active job when necessary, detaches it from the persisted session, and resets the workspace; accepting a proposal also detaches it. Explicit **Cancel search** stops the job while leaving its progress visible until it is cleared. Missing movie, complete-series, and season targets can be retried independently without discarding successful proposal rows.
